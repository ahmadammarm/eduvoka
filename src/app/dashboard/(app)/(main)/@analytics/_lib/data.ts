...
import { BurnoutLevel } from "@/types/burnout";
...
export async function getBurnoutTrend(): Promise<BurnoutTrend> {
	const userId = await getUserId();
	if (!userId) {
		return {
			avgBurnoutIndex: 0,
	...
	// ✅ Fetch LatihanSession dengan SessionMetrics
	const sessionsWithMetrics = await prisma.latihanSession.findMany({
		where: {
			userId,
			endedAt: { not: null },
			startedAt: { gte: thirtyDaysAgo },
			// ✅ Check if SessionMetrics exists
	...
	// Calculate average
	const totalFatigue = sessionsWithMetrics.reduce(
		(sum, s) => sum + (s.sessionMetrics?.fatigueIndex || 0),
		0
	);
	const avgBurnoutIndex = totalFatigue / sessionsWithMetrics.length;
	...
	sessionsWithMetrics.forEach(s => {
		const level = s.sessionMetrics?.burnoutLevel || 'NONE';
	...
	const recent = sessionsWithMetrics.filter(s => s.startedAt >= sevenDaysAgo);
	const previous = sessionsWithMetrics.filter(
		s => s.startedAt < sevenDaysAgo
	);

	const recentAvg = recent.length > 0
		? recent.reduce((sum, s) => sum + (s.sessionMetrics?.fatigueIndex || 0), 0) / recent.length
		: 0;
	const previousAvg = previous.length > 0
		? previous.reduce((sum, s) => sum + (s.sessionMetrics?.fatigueIndex || 0), 0) / previous.length
		: 0;

	let trend: 'IMPROVING' | 'WORSENING' | 'STABLE' = 'STABLE';
	if (Math.abs(recentAvg - previousAvg) > 5) {
		trend = recentAvg < previousAvg ? 'IMPROVING' : 'WORSENING';
	}

	// Recent sessions
	const recentSessions = sessionsWithMetrics.slice(-10).map(s => ({
		date: formatDateStr(s.startedAt),
		burnoutLevel: (s.sessionMetrics?.burnoutLevel || 'NONE') as BurnoutLevel,
		fatigueIndex: s.sessionMetrics?.fatigueIndex || 0
	}));

	return {
		avgBurnoutIndex: Number(avgBurnoutIndex.toFixed(1)),
		distribution: distribution as Record<BurnoutLevel, number>,
		trend,
		recentSessions
	};
}

// Update getAnalyticsData
